...
-- Política para subir imágenes (solo usuarios autenticados)
CREATE POLICY "Negocios pueden subir imágenes de sus servicios y productos"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
...
-- Política para leer imágenes (público)
CREATE POLICY "Cualquiera puede ver imágenes de los servicios y productos"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'service_images');

-- Política para eliminar imágenes (solo propietario)
CREATE POLICY "Negocios pueden eliminar las imágenes de sus servicios y productos"
ON storage.objects FOR DELETE
TO authenticated
USING (
  bucket_id = 'service_images' AND
